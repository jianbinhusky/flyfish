    ...
    this.events = function() {
        ...
        $('.lj_btn').on('click', function() {
            ...
            if (type == 'question') {
                ...
                maptss("逻辑设置",'/edit/ajax/option_jump_set/'+get_oid(project)+
                    '/?option_id='+oid+'&pid='+get_oid(project)+'&ts='+(new Date()).getTime()
                    , "500");
            }
            return false;
        });
        ...
        $('.yy_btn').on('click', function() {
            ...
            maptss("引用设置",'/edit/ajax/autoreplace_set/'+get_oid(project)+
                '/?oid='+valid+'&pid='+get_oid(project)+'&type='+type+'&ts='+(new Date()).getTime(),"485");
