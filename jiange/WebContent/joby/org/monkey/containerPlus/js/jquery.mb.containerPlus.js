...
(function(jQuery){
	...
	jQuery.containerize={
		...
		build:function(el){
			...
			var $footerhtml = el.$.find(".modal-footer:first");
			$footerhtml.remove();
			var HTML = el.$.html();
			el.$.empty();
			...
			var header = jQuery("<div/>").addClass("mbc_header modal-header");
			...
			var footer = jQuery("<div/>").addClass("mbc_footer modal-footer").unselectable().html($footerhtml.html());;
		...
		methods:{
			...
			resize:function(){
				jQuery.cMethods.resize = {name: "resize", author:"pupunzi", type:"built-in"};// el.$.css("position","absolute")
				var el = this;
				el.position = el.$.data("drag") ?
						(el.$.css("position") == "relative" || el.$.css("position") == "static") ? el.$.css("position","fixed")
			...
			open:function(animate,btf){
				...
				var time= animate ? animate : 0;
				if(typeof el.opt.onBeforeOpen === "function")
					el.opt.onBeforeOpen(el);
				if(el.isClosed)
			...
			adjust:function(){
				...
				//if(h<150) h=150;
				//el.$.find(".mbc_content").css({height:h-84});
			...
			alwaisontop:function(){
				...
				el.$.css("z-index",1002).addClass("alwaysOnTop");
			},
			...
			fullScreen:function(){
				...
				if(!el.fullscreen){
					...
					el.oMaxHeight= el.$.css("max-height");
					...
					el.$.css({top:el.oTop,left:el.oLeft, width:el.oWidth, height:el.oHeight, position: el.oPos,maxHeight:el.oMaxHeight});
					el.$.containerize("adjust");
					el.fullscreen=false;
