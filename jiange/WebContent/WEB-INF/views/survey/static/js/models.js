...
QBase.prototype.estimate_time = function() {
    var option_count = isNotEmpty(this.question.option_list) ? this.question.option_list.length : 0;
    var row_count = isNotEmpty(this.question.matrixrow_list) ? this.question.matrixrow_list.length : 0;
    var cost_time = null;
    if (check_in(this.question.question_type, [QUESTION_TYPE_MATRIX_SCORE])){
        cost_time = strip_tag(this.title).length * 0.08 + option_count * row_count * 1.3;
    }else{
        cost_time = strip_tag(this.title).length * 0.08 + option_count * 1.3 + row_count * 2.5;
    }

    if (cost_time < 2){
        cost_time = 2;
    }
    if (check_in(this.question.question_type, [QUESTION_TYPE_BLANK, QUESTION_TYPE_MULTIPLE_BLANK, QUESTION_TYPE_MATRIX_BLANK])){
        cost_time += 5;
    }
    return parseInt(cost_time);
};
